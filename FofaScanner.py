...
FOFA_Email = 'XXXXXX@XXX.com'    # FOFA注册邮箱
FOFA_Key = 'XXXXXXXXXXXXXXX'   # FOFA API_Key

params = {
    'email': FOFA_Email,
    'key': FOFA_Key,
    'size': '100',   # FOFA官方为100条(每次查询返回记录数，默认为100条，最大可设置为10000条)
    'page': '1'      # FOFA官方为第一页(翻页数，默认为第一页)
}
...
def get_result():
    ...
    if '820000' in response.text:
        ...
        choice = input("当前结果共有"+result_num+"个,是否下载结果至本地?(y/n)")
        if choice == 'n' or choice == 'N' or choice =='no' or choice == 'No':
            exit()
        else:
            if not os.path.exists('/results'):  # 检查results文件夹是否存在，不存在则创建
                os.mkdir('/results')
            file_name = "./results/"+str(datetime.datetime.now().month)+"-"+str(datetime.datetime.now().day)+".txt"
